...
bool pg_jsonapi::DocumentConfig::LoadConfigFromDB (bool& o_config_exists)
{
    ...
    if ( ! OidIsValid(s_oid) ) {
        ereport(LOG, (errmsg_internal("jsonapi: cannot load configuration for URL '%s' ('public' schema does not exist)",
                                      base_url_.c_str())));
        return true; // configuration is not mandatory
    } else {
        relid = get_relname_relid( "jsonapi_config", s_oid);
        if ( ! OidIsValid(relid) ) {
            ereport(LOG, (errmsg_internal("jsonapi: cannot load configuration for URL '%s' ('public.jsonapi_config' does not exist)",
                                          base_url_.c_str())));
            return true; // configuration is not mandatory
        }
    }
